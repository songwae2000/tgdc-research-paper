...
longer than the median. Let \texttt{A} denote the AUC attained by an arm and \texttt{C} the
ceiling AUC available from the same columns; recovered skill, reported alongside
AUC, is then \texttt{S = (A - 0.5) / (C - 0.5)}. The null tested is that the rules
...
The first recorded hypothesis, fixed in \texttt{HYPOTHESIS.md} before any run, was that
...
utility of data altered to protect confidentiality. \textit{The American Statistician},
60(3):224-232, 2006.
...
for inference with clustered errors. \textit{Rev. Econ. Stat.}, 90(3):414-427, 2008.
...
release via Bayesian networks. \textit{ACM Trans. Database Syst.}, 42(4):25, 2017.
